...
## Известные проблемы
Есть принципиальная проблема с сбросом CH446Q при включении питания.<br>
При включении питания Gluk стабильно выводит экран Help, как будто зажата кнопка, т.е. предположительно не происходит корректный аппаратный сброс матрицы ключей CH449Q.<br>
После сброса Z80 старт происходит корректно, т.к. MCU отрабатывает код программного сброса состояния матрицы клавиатуры.<br>

Используется проверенная схема сброса из [zx-rgbi-to-vga-hdmi_gotek-osd](https://github.com/proboterror/zx-rgbi-to-vga-hdmi_gotek-osd)<br>
В цепочке сброса CH446Q стоят резистор 10K и конденсатор 0.1uF.<br>
По формуле это дает длительность сигнала сброса порядка 1 ms, конденсатор 1uF не меняет поведение.<br>
На Pentagon 2024 Gluk производит первый опрос клавиатуры через 24 ms после появления питания.<br>
Сигнал на выходе MCU CH559L на старте main() при использовании внутреннего power on reset появляется через 57 ms после появления питания, т.е. формировать сброс CH446Q через MCU не вариант.
...
- Заказ плат после решения проблемы сброса матрицы CH449Q при включении питания / ревью буферов сигналов Z80 на транзисторах / ревью цепи сброса CPLD.
