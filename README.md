...
- Реализована поддержка геймпадов/джойстиков, совместимых с USB HID.
- Реализованы специальные драйверы для геймпадов Sony DualShock 3, XBox 360 USB Controller, XBox One Controller.
...
- Драйвер геймпада XBox One не тестировался.
<hr>
...
Сигналы PAUSE, MAGIC, RESET от CPLD буферизированы на сигналы шины Z80 BUSRQ/, NMI/, RST через FET транзисторные ключи BSS123. Требует review и проверки.
...
Сброс происходит по фронту сигнала (более правильно было бы по уровню сигнала). Возможно значение конденсатора / резистора цепи формирования RST_IN потребует подбора.
...
- Платы собраны и тестируются.

Раздельно протестированы контроллеры Kempston Mouse, Kempston Joystick, ZX матричной клавиатуры: работают, корректно сбрасываются при включении. Реализация матрицы клавиатуры позволяет зажимать много кнопок без эффекта фантомных нажатий, в том числе при маппинге с HID геймпада.<br>
При подключении к шине данных через описание в Verilog любых двух из трех устройств работает без замечаний.<br>
При подключении к шине данных трех устройств тенденция к мусору в порту клавиатуры.<br>
Работа сигналов PAUSE, MAGIC, RESET через транзисторные ключи на шину не проверена.
...
- Для CH559L заявлена поддержка serial programmer, доступен инструмент [CH55x_python_flasher](https://github.com/hexeguitar/CH55x_python_flasher) для Windows / Linux, не тестировалось.
